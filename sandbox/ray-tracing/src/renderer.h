...
private:
    ...
    for (int i = 0; i < MAX_SPHERES; i++) {
        ...
        if (densityDist(gen) < 0.3f) {
            if (!spheres.empty()) {
                ...
                sphere.centerX = spheres[clusterTarget].centerX + std::uniform_real_distribution<float>(-8.0f, 8.0f)(gen);
                sphere.centerY = spheres[clusterTarget].centerY + std::uniform_real_distribution<float>(-3.0f, 3.0f)(gen);
                sphere.centerZ = spheres[clusterTarget].centerZ + std::uniform_real_distribution<float>(-5.0f, 5.0f)(gen);
            }
        }
  ...
  void updateSphereBuffer() {
    ...
    if (!spheres.empty()) {
        glBufferSubData(GL_SHADER_STORAGE_BUFFER, sphereDataOffset, 
                        sphereDataSize, packedSpheres.data());
    }
  ...
  aabbMin = glm::vec3(
    spheres[0].centerX - spheres[0].radius,
    spheres[0].centerY - spheres[0].radius,
  ...
  for (size_t i = 1; i < spheres.size(); i++) {
    glm::vec3 sphereMin(
      spheres[i].centerX - spheres[i].radius,
      spheres[i].centerY - spheres[i].radius,
